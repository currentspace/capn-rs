...
import { InteropTestFramework, InteropAssert, Timer } from './test-framework.js';

var __defProp = Object.defineProperty;
var __name = (target, value) => __defProp(target, "name", { value, configurable: true });
const RUST_SERVER_URL = "ws://localhost:8080/ws";
const TEST_TIMEOUT = 1e4;
class TypeScriptClientTests {
  static {
    __name(this, "TypeScriptClientTests");
  }
  framework;
  client = null;
  transport = null;
  ...
  async runAllTests() {
    console.log("\u{1F525} TypeScript Client \u2192 Rust Server Interoperability Tests");
    console.log("\u{1F4E1} Testing TypeScript client against Rust server...");
    try {
  ...
  async testBasicCalculatorOperations() {
    await this.setupClient();
    const addResult = await this.client.call(1, "add", [15.5, 24.3]);
    InteropAssert.approximatelyEqual(addResult, 39.8, 0.01, "Addition result should be correct");
  ...
  async testCalculatorErrorHandling() {
    await this.setupClient();
    try {
  ...
  async testMultipleSequentialCalls() {
    await this.setupClient();
    const operations = [
  ...
  async testAdvancedMathOperations() {
    await this.setupClient();
    ...
    InteropAssert.approximatelyEqual(sqrtDecimalResult, 1.4142135623730951, 1e-4, "Square root of 2 should be correct");
  }
  async testFactorialOperations() {
    await this.setupClient();
  ...
  async testUserManagement() {
    await this.setupClient();
    const user1 = await this.client.call(100, "getUser", [1]);
    ...
    const user2 = await this.client.call(100, "getUser", [2]);
  ...
  async testCreateUser() {
    ...
    const newUser = await this.client.call(100, "createUser", [userData]);
  ...
  async testPerformance() {
    ...
    const { result, duration } = await Timer.measure(async () => {
      const promises = Array.from(
        { length: 100 },
        (_, i) => this.client.call(1, "add", [i, i])
      );
    ...
    InteropAssert.ok(duration < 5e3, "Should complete within 5 seconds");
  }
  async testHighVolumeOperations() {
    ...
    InteropAssert.ok(duration < 1e4, "Should complete within 10 seconds");
  }
  // Edge Cases
  async testLargeNumbers() {
    ...
    const smallNum1 = 1e-6;
    const smallNum2 = 2e-6;
    const smallResult = await this.client.call(1, "add", [smallNum1, smallNum2]);
    InteropAssert.approximatelyEqual(smallResult, 3e-6, 1e-7, "Should handle small numbers correctly");
  }
  async testInvalidArguments() {
    await this.setupClient();
    try {
...
__name(runClientTests, "runClientTests");
