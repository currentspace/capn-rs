...
use capnweb_core::{
    ...
    protocol::{
        ...
        DefaultNestedCapableTarget, ExportTable, IdAllocator, ImportTable, MethodMetadata,
        NestedCapableRpcTarget, ParameterMetadata, PersistentSessionManager, PlanBuilder,
        PlanRunner, ResumeTokenManager, VariableStateManager,
    },
    CapId, RpcError, RpcTarget,
};
        ...
        match method {
            ...
            "getHistory" => {
                ...
                let history_values: Vec<Value> =
                    history.iter().map(|s| Value::String(s.clone())).collect();
                Ok(Value::Array(history_values))
            }

            "getPrecision" => Ok(Value::Number(Number::from(self.precision))),
        ...
        match capability_type {
            "calculator" => {
                let precision = match config {
                    Value::Object(ref obj) => match obj.get("precision") {
...
async fn test_comprehensive_advanced_features_integration() {
    ...
    println!(
        "✅ Resume token created successfully: {}",
        &token.session_id
    );

    // Restore the session
    let restored_session_id = session_manager
        .restore_session(
            &token,
            &allocator,
            &imports,
    ...
    let sub_cap_result = nested_target
        .create_sub_capability("advanced_calculator", config)
        .await;
    ...
    assert!(
        ...
        "Graph should have capabilities"
    );
    ...
    let multiply_result = builder.add_call(
        Source::capture(cap_index),
        "multiply".to_string(),
        vec![Source::result(add_result), Source::by_value(json!(2))],
    );
    ...
    fields.insert(
        "operation".to_string(),
        Source::by_value(json!("Complex calculation")),
    );
    ...
    assert!(
        complexity.call_operations > 0,
        "Should have call operations"
    );
    assert!(
        complexity.object_operations > 0,
    ...
    let nested_result = nested_calc
        .create_sub_capability("calculator", sub_config)
        .await;
    ...
    let invalid_execution = runner
        .execute_plan(
            &invalid_plan,
            Value::Object(std::collections::HashMap::new()),
            vec![Arc::new(MockRpcTarget::new())],
        )
        .await;
    ...
    assert!(
        ...
        "Should fail with invalid capability type"
    );
...
async fn test_resume_token_persistence_and_recovery() {
    ...
    println!(
        "✅ Session variables preserved: {} items",
        restored_snapshot.variables.len()
    );
...
async fn test_nested_capability_lifecycle() {
    ...
    let configs = vec![("calc1", 2), ("calc2", 4), ("calc3", 8)];
    ...
    if let Some(id_to_dispose) = created_ids.first() {
        let dispose_result = nested_target
    ...
    println!(
        "✅ Graph now contains {} capabilities after disposal",
        stats_after.total_capabilities
    );
...
async fn test_il_plan_runner_edge_cases() {
    ...
    // Test 1: Empty plan
    let empty_plan = Plan::new(vec![], vec![], Source::by_value(json!("empty")));

    let empty_result = runner
        .execute_plan(
            &empty_plan,
            Value::Object(std::collections::HashMap::new()),
            vec![],
    ...
    let param_execution = runner
        .execute_plan(&plan, parameters, vec![calc_target.clone()])
        .await;
    assert!(
        param_execution.is_ok(),
    ...
    let val1 = complex_builder.add_call(
        Source::capture(cap_index),
        "add".to_string(),
        vec![Source::by_value(json!(1)), Source::by_value(json!(2))],
    );

    let val2 = complex_builder.add_call(
        Source::capture(cap_index),
        "multiply".to_string(),
        vec![Source::result(val1), Source::by_value(json!(3))],
    );
    ...
    assert!(
        complexity.call_operations >= 2,
        "Should have call operations"
    );
    assert!(
        complexity.object_operations >= 2,
        "Should have object operations"
    );
    assert!(
        complexity.array_operations >= 1,
